...
class Auth
{
     ...
     */
    public function __construct(\PDO $dbh, $config, $language = "ko_KR")
    {
        $this->dbh = $dbh;
        $this->config = $config;
     ...
     * @return array $return
     */
    public function login($id, $password, $remember = 0, $captcha = NULL)
    {
        $return['error'] = true;
        ...
        $validateid = $this->validateId($id);
        $validatePassword = $this->validatePassword($password);

        if ($validateid['error'] == 1) {
            $this->addAttempt();
            $return['message'] = $this->lang["id_password_invalid"];
        ...
        $uid = $this->getUIDbyID(strtolower($id));
    ...
    */
    ...
    public function register($id, $email, $password, $repeatpassword, $params = Array(), $captcha = NULL, $sendmail = true)
    {
        $return['error'] = true;
        $block_status = $this->isBlocked();
        ...
        if($validateid['error'] == 1){
            $return['message'] = $validateid['message'];
        ...
        if ($this->isIdTaken($id)) {
            $this->addAttempt();
            $return['message'] = $this->lang["id_taken"];

            return $return;
        }

        $addUser = $this->addUser($id, $email, $password, $params, $sendmail);
    ...
    */
    ...
    {
	$query = $this->dbh->prepare("SELECT id FROM {$this->config->table_users} WHERE userid = ?");
    ...
    */
    ...
    public function isIdTaken($id)
    {
        $query = $this->dbh->prepare("SELECT count(*) FROM {$this->config->table_users} WHERE userid = ?");
        $query->execute(array($id));
    ...
    */
    ...
    protected function addUser($id, $email, $password, $params = array(), &$sendmail)
    {
        $return['error'] = true;
        ...
        $query = $this->dbh->prepare("UPDATE {$this->config->table_users} SET userid = ?, email = ?, password = ?, isactive = ? {$setParams} WHERE id = ?");

        $bindParams = array_values(array_merge(array($id, $email, $password, $isactive), $params, array($uid)));
    ...
    */
    ...
    {
        $query = $this->dbh->prepare("SELECT userid, email, password, isactive FROM {$this->config->table_users} WHERE id = ?");
        $query->execute(array($uid));
    ...
    */
    ...
    protected function validateId($id) {
        ...
        if (strlen($id) < (int)$this->config->verify_id_min_length ) {
            $return['message'] = $this->lang["id_short"];

            return $return;
        } elseif (strlen($id) > (int)$this->config->verify_id_max_length ) {
            $return['message'] = $this->lang["id_long"];
    ...
    */
    ...
    public function resendActivation($email, $sendmail = true)
    {
        $return['error'] = true;
        $block_status = $this->isBlocked();
